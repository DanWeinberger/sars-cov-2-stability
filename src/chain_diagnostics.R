...
#for (k in 1:length(mcmc_fit_paths)) {
   ...
   # models[k] <- model_name
    rhat_max_parms <- rhat_maxes[[1]]
    rhat_max_vals <- rhat_maxes[[2]]
    neff_min_parms <- neff_mins[[1]]
    neff_min_vals <- neff_mins[[2]]
#}
...
write.csv(diagnostic_data, 'out/chain_diagnostics.csv')
