...
TODO: censoring analysis (set first negative point as censor date)
...
mod4 <- lm(log10_titer ~ 0 + time + humidity + surface + variant+ time*humidity, data=a1.m)  #+ (time|unique_replicate), data=a1.m)

  pred.coefs.reg.mean4 <-
    mvrnorm(n = 10000,
            mu = mod3$coefficients,
            Sigma = vcov(mod3))
  
  
t_half_30 <- log10(0.5)/(pred.coefs.reg.mean4[,'time'])

t_half_60 <- log10(0.5)/(pred.coefs.reg.mean4[,'time']  + pred.coefs.reg.mean4[,'time:humidity0.6'])
